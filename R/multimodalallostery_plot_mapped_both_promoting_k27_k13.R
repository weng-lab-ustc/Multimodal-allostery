...
#'
#' Workflow-only September 2026 implementation of `multimodalallostery_plot_mapped_both_promoting_k27_k13()`.
#'
#' @details Scientific logic follows the current `confirm_scriptcode_202609` workflow. Provenance and old/new comparison are recorded in `FUNCTION_INVENTORY_202609.csv`.
#' @param k27_k13_data Input or option used by this workflow; see Usage for the exact interface.
...
#' @export
multimodalallostery_plot_mapped_both_promoting_k27_k13 <- function (k27_k13_data, both_promoting_muts, xlim = c(-1.5, 3), ylim = c(-1.5, 3), fixed_threshold = 0.40000000000000002) 
{
    merged_data <- data.table::copy(k27_k13_data$data)
    ...
    cat("\nPlotting thresholds for K27 vs K13:\n")
    ...
    cor_test <- stats::cor.test(merged_data$ddG_K27, merged_data$ddG_K13)
    ...
    p <- ggplot2::ggplot() + ggplot2::theme_classic(base_size = 20) + ggplot2::geom_vline(xintercept = c(-threshold_K27, 
        threshold_K27), linetype = "dashed", color = "grey50", size = 0.80000000000000004) + ggplot2::geom_hline(yintercept = c(-threshold_K13, 
        threshold_K13), linetype = "dashed", color = "grey50", size = 0.80000000000000004) + ggplot2::geom_point(data = merged_data[is_target == 
        FALSE], ggplot2::aes(ddG_K27, ddG_K13), color = "grey80", size = 2, alpha = 0.40000000000000002, shape = 16) + ggplot2::geom_point(data = target_non_nbp, 
        ggplot2::aes(ddG_K27, ddG_K13), color = "#FFB0A5", shape = 16, size = 3) + ggplot2::geom_point(data = target_nbp, 
        ggplot2::aes(ddG_K27, ddG_K13), color = "#FFB0A5", shape = 17, size = 3.5) + ggplot2::annotate("text", x = xlim[1] + 
        0.10000000000000001, y = ylim[2] - 0.10000000000000001, hjust = 0, vjust = 1, label = r_label, size = 6) + ggplot2::labs(x = expression("Binding" ~ 
        Delta * Delta * G ~ "(K27) (kcal/mol)"), y = expression("Binding" ~ Delta * Delta * G ~ "(K13) (kcal/mol)"), title = "K13 vs K19 both promoting mutations mapped onto K27 vs K13") + 
