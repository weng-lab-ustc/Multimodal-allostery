...
#' @export
multimodalallostery_calculate_site_threshold <- function(data, assay_sele, anno) {
    anno_data <- fread(anno)
    data_no_wt <- data[mt_codon != "WT"]
    site_ddG <- data_no_wt[, .(mean_ddG = sum(abs(ddG)/ddG_std^2, na.rm = TRUE)/sum(1/ddG_std^2, na.rm = TRUE)), by = Pos_real]
    site_sigma <- data_no_wt[, .(sigma = sqrt(1/sum(1/ddG_std^2, na.rm = TRUE))), by = Pos_real]
