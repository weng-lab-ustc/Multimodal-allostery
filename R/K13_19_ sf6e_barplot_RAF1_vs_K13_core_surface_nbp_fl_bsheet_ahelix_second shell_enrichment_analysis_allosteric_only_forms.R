...
run_pair_all_regions <- function(pair_name, input_files, anno, structure_regions, second_shell_map) {
  ...
  if(length(second_shell_x) > 0) {
    ...
    or_list_shell_x <- lapply(categories_to_test, function(cat) {
      res <- calc_or_original(df, second_shell_x, cat)
      data.table(
        pair = pair_name,
  ...
  if(length(second_shell_y) > 0) {
    ...
    or_list_shell_y <- lapply(categories_to_test, function(cat) {
      res <- calc_or_original(df, second_shell_y, cat)
      data.table(
        pair = pair_name,
...
# =========================================================
# 创建图：包含 Allosteric only in X, Allosteric only in Y, Other
# =========================================================
...
ggsave(paste0(output_dir, "barplot_RAF1_vs_K13_allosteric_only_with_second_shell2.pdf"),
       plot = p,
       width = 10,
       height = 6,
       device = cairo_pdf)
