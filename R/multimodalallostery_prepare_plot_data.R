#' prepare plot data
#'
#' Workflow-only September 2026 implementation of `multimodalallostery_prepare_plot_data()`.
#'
#' @details Scientific logic follows the current `confirm_scriptcode_202609` workflow. Provenance and old/new comparison are recorded in `FUNCTION_INVENTORY_202609.csv`.
#' @param NBP_results Input or option used by this workflow; see Usage for the exact interface.
#' @param second_shell_results Input or option used by this workflow; see Usage for the exact interface.
#' @param beta_sheet_results Input or option used by this workflow; see Usage for the exact interface.
...
multimodalallostery_prepare_plot_data <- function (NBP_results, second_shell_results, beta_sheet_results, assay_name) 
{
    ...
    nbp_data <- data.table::data.table(Region = "NBP", Category = c("Allosteric", "Inhibit", "Stabilize"), Cases_in_region = c(NBP_results$Case_in_NBP[1], 
    ...
    if (!is.null(second_shell_results) && nrow(second_shell_results) > 0) {
        shell_data <- data.table::data.table(Region = "Second shell", Category = c("Allosteric", "Inhibit", "Stabilize"), 
            Cases_in_region = c(second_shell_results$Case_in_region[1], second_shell_results$Case_in_region[2], second_shell_results$Case_in_region[3]), 
            OR = second_shell_results$OR, P_value = second_shell_results$P_value)
    ...
    if (!is.null(beta_sheet_results) && nrow(beta_sheet_results) > 0) {
        beta_data <- data.table::data.table(Region = "Beta sheet", Category = c("Allosteric", "Inhibit", "Stabilize"), Cases_in_region = c(beta_sheet_results$Case_in_region[1], 
    ...
    plot_data[, `:=`(Significance, ifelse(P_value < 0.050000000000000003, ifelse(P_value < 0.01, ifelse(P_value < 0.001, 
        "***", "**"), "*"), " ns"))]
