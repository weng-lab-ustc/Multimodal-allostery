#' prepare mapped merged data with fdr
#'
#' Workflow-only September 2026 implementation of `multimodalallostery_prepare_mapped_merged_data_with_fdr()`.
#'
...
#' @export
multimodalallostery_prepare_mapped_merged_data_with_fdr <- function (input_x, input_y, assay_x, assay_y, anno, binding_sites_map = list(RAF1 = c(21, 25, 29, 31, 33, 36, 37, 38, 39, 
    40, 41, 67, 71), K27 = c(21, 24, 25, 27, 31, 33, 36, 38, 39, 40, 41, 43, 52, 54, 67, 70, 71), K13 = c(63, 68, 87, 88, 
    90, 91, 92, 94, 95, 96, 97, 98, 99, 101, 102, 105, 106, 107, 129, 133, 136, 137, 138), K19 = c(68, 87, 88, 90, 91, 92, 
    94, 95, 97, 98, 99, 101, 102, 105, 107, 108, 125, 129, 133, 136, 137)), fixed_threshold = 0.40000000000000002) 
{
    data_x <- multimodalallostery_load_mapped_mutation_data(input = input_x, assay_sele = assay_x)
    data_y <- multimodalallostery_load_mapped_mutation_data(input = input_y, assay_sele = assay_y)
    threshold_x <- multimodalallostery_calculate_threshold(data = data_x, assay_sele = assay_x, anno = anno, fixed_threshold = fixed_threshold)
    threshold_y <- multimodalallostery_calculate_threshold(data = data_y, assay_sele = assay_y, anno = anno, fixed_threshold = fixed_threshold)
    ...
    return(list(data = merged_data, threshold_x = threshold_x, threshold_y = threshold_y))
}
