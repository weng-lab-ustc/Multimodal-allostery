...
#'
#' @param k27_k19_data Value supplied for `k27_k19_data`.
...
#' @export
multimodalallostery_plot_mapped_both_promoting_k27_k19 <- function(k27_k19_data, both_promoting_muts, xlim = c(-1.5, 3), ylim = c(-1.5, 3)) {
    ...
    p <- ggplot() + theme_classic(base_size = 20) + geom_vline(xintercept = c(-threshold_K27, threshold_K27), linetype = "dashed", 
        ...
        geom_point(data = merged_data[is_target == FALSE], aes(ddG_K27, ddG_K19), color = "grey80", size = 2, alpha = 0.4, 
            shape = 16) + geom_point(data = target_non_nbp, aes(ddG_K27, ddG_K19), color = "#FFB0A5", shape = 16, size = 3) + 
        geom_point(data = target_nbp, aes(ddG_K27, ddG_K19), color = "#FFB0A5", shape = 17, size = 3.5) + annotate("text", 
        x = xlim[1], y = ylim[2], hjust = 0, vjust = 1, label = r_label, size = 6) + labs(x = expression("Binding" ~ Delta * 
        Delta * G ~ "(K27) (kcal/mol)"), y = expression("Binding" ~ Delta * Delta * G ~ "(K19) (kcal/mol)"), title = "K13 vs K19 both promoting mutations mapped onto K27 vs K19") + 
