#' load k27 vs k13 data
#'
#' Workflow-only September 2026 implementation of `multimodalallostery_load_k27_vs_k13_data()`.
#'
...
#' @export
multimodalallostery_load_k27_vs_k13_data <- function (input_k27, input_k13, anno, binding_sites_map = list(RAF1 = c(21, 25, 29, 31, 33, 36, 37, 38, 39, 40, 41, 67, 71), 
...
{
    input_files <- list(K27 = input_k27, K13 = input_k13)
    anno <- anno
    prepared <- multimodalallostery_prepare_mapped_merged_data_with_fdr(input_x = input_files$K27, input_y = input_files$K13, 
        assay_x = "K27", assay_y = "K13", anno = anno, binding_sites_map = binding_sites_map, fixed_threshold = fixed_threshold)
    cat("\nK27 vs K13 thresholds:\n")
    cat("  K27 threshold:", prepared$threshold_x, "\n")
    cat("  K13 threshold:", prepared$threshold_y, "\n")
    return(list(data = prepared$data, threshold_K27 = prepared$threshold_x, threshold_K13 = prepared$threshold_y))
}
