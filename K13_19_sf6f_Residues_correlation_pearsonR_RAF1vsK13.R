...
# ============================================================================
# Step 2. Structural annotations (完整版 - 包含所有6个β-strands)
# ============================================================================
...
p_R <- ggplot(
  ...
  labs(
    ...
    y = "Residue Correlation Pearson's R between RAF1 and K13",
...
# 使用rev()使得显示顺序为：BI1在最上面，γ-phosphate contact在最下面
annotation_df$track <- factor(
  
  annotation_df$track,
  
  levels = rev(names(annotation_tracks))  # rev()让第一个显示在顶部
  
)

annotation_df$residue_order <- factor(
  
  annotation_df$residue_order,
  
  levels = levels(plot_df$residue_order)
...
p_annotation <- ggplot(
  ...
  labs(
    ...
    x = "KRAS residue position (ordered by Pearson's R)",
...
ggsave(
  ...
  "C:/Users/36146/OneDrive - USTC/Manuscripts/K13_K19/figures/20260521_start_Updating/result5_figure6/PanelC_PearsonR_annotation5.pdf",
